...
function build_adjacency(triangles) {
...
function find_containing_triangle(triangles, point) {
    for (let i = 0; i < triangles.length; i++) {
...
function triangle_centroid(triangle) {
...
function dijkstra(triangles, adjacency, sourceIdx, targetIdx) {
...
// Create corridor
function create_corridor(triangles, path) {
    const corridor = [];
...
function funnel_path(start, end, corridor) {
    let path = [start];
    let apex = start;
    let left = start;
    let right = start;
...
function geodesic_path(polygon, source, target) {
    // Step 1: Triangulate the polygon
...
function geodesic_distance(polygon, source, target) {
